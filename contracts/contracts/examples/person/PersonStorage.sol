...
import './PersonLib.sol';

contract PersonStorage is StorageBase {
    using PersonLib for PersonLib.Person;

    mapping(bytes32 => Type) public typeStruct;

    struct Type {
        PersonLib.Person data;
        uint256 index;
    }
    ...
    function insert(PersonLib.Person memory data) public returns (bytes32 hash) {
        // for data integrity
    ...
    function update(bytes32 hashi, PersonLib.Person memory data)
        public
    ...
    function getByHash(bytes32 hash) public view returns(PersonLib.Person memory data) {
