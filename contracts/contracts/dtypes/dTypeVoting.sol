...
contract TypeVotingContract {
    ...
    struct Type {
        dTypeVotingLib.TypeVoting data;
        uint256 index;
    }
    ...
    function insert(dTypeVotingLib.TypeVoting memory data) public returns (bytes32 hasho) {
        ...
        typeStruct[hash].data.proposal = data.proposal;

        uint256 length = data.options.length;

        for(uint256 i = 0; i < length; i++) {
            typeStruct[hash].data.options.push(data.options[i]);
        }
    ...
    function insertBytes(bytes memory data) public returns (bytes32 hasho) {
        return insert(dTypeVotingLib.structureBytes(data));
    }
    ...
    function update(bytes32 hashi, dTypeVotingLib.TypeVoting memory data)
        public
    ...
    function getByHash(bytes32 hash) public view returns(dTypeVotingLib.TypeVoting memory data) {
